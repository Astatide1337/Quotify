...
# def TTS(text: str, filename):
#     if platform.system() == "Windows":
#         command = f'echo "{text}" | Piper-TTS\\piper.exe -m Piper-TTS\\en_US-norman-medium.onnx -c Piper-TTS\\en_US-norman-medium.onnx.json -f {filename}'
#     else:
#         command = f'echo "{text}" | ./Piper-TTS/piper.exe -m ./Piper-TTS/en_US-norman-medium.onnx -c ./Piper-TTS/en_US-norman-medium.onnx.json -f {filename}'
    
#     result = os.system(command)
#     if result != 0:
#         print("An error occurred while executing the TTS command.")

def TTS(text:str, speaker:str, filename:str):
    try:
        os.system(f'tts --model_name tts_models/multilingual/multi-dataset/xtts_v2 --text "{text}" --speaker_wav ./Speakers/{speaker}.wav --language_idx en --use_cuda true --out_path "{filename}"')
    except:
...
def DownloadMusic(folder: str) -> None:
    ...
    try:
        ...
        while videos:
            ...
            options = [
                'yt-dlp',
                '-f', 'bestaudio[ext=m4a],bestaudio[ext=webm]',
                '-x',
                '--audio-format', 'mp3',
                '--output', f"./Music/{folder} Music/%(title)s.%(ext)s"
            ]
    ...
    for url in urls:
        ...
        if response.status_code == 200:
            ...
            if url == "https://zenquotes.io/api/random":
                dynamicPoems.append(data[0]['q'])
            elif url == "https://programming-quotesapi.vercel.app/api/random":
                dynamicPoems.append(data['quote'])
        else:
                            ...
                            if isinstance(data, (dict, list)):
                                ...
                                for item in data:
                                    if isinstance(item, dict):
                                        ...
                                        if quote and author:
                                            folder_quotes[path.name].append({"quote": quote, "author": author})
                                        
                    except (IOError, json.JSONDecodeError) as e:
                        logging.error(f"Error reading {file_path}: {e}")
    
    staticPoems = {folder: random.choice(quotes) for folder, quotes in folder_quotes.items() if quotes}
    
    return staticPoems


def CreateVideo(wav, subtitles, title:str, genre):
    MUSIC = os.path.join("Music", f"{genre} Music", random.choice(os.listdir(os.path.join("Music", f"{genre} Music"))))
    ...
    repeat = int(wavDuration / videoDuration)
...
def Subtitles(filename):
    if not os.path.exists(f'./Subtitles/{filename.replace("WAVFILES/", "").replace(".wav", "")}.srt'):
        try:
            subprocess.run(
                    ["auto_subtitle_llama", f"{filename}", "--model", "medium", "-o", "./Subtitles/"]
                )
        except:
            print("Error generating subtitles on video")
    else:
        os.remove(f'./Subtitles/{filename.replace("WAVFILES/", "").replace(".wav", "")}.srt')
        try:
            subprocess.run(
                    ["auto_subtitle_llama", f"{filename}", "-o", "./Subtitles/"]
                )
        except:
            print("Error generating subtitles on video")
...
def Dynamic(speaker):
    ...
    for poem in DYNAMIC:
        quote = poem['quote']
        author = poem['author']
        filename = "./WAVFILES/{quote.replace(" ","")}.wav"
...
def Static(speaker):
    ...
    for folder, poem in STATIC.items():
        ...
        CreateVideo(STATIC[folder]["filename"], f"./Subtitles/{filename.replace('WAVFILES/', '').replace('.wav', '')}.srt", f"{quote} - {author}", folder.replace(" Quotes", ""))

def main():
    CreateVideo("./WAVFILES/GeneralQuotesEdwardEricson.wav", "./Subtitles/GeneralQuotesEdwardEricson.srt", "It is fatal to enter any war without the will to win it. - General Douglas MacArthur.mp4", "General")
...
On Run:
Create a TTS for General
Create a TTS for Programming
Create a TTS for Anime


TODO:
Add to Static: The Anime and then pull clip from that anime for the background.


CHANGE TTS TO THIS: tts --model_name tts_models/multilingual/multi-dataset/xtts_v2 --text "And my dream is to be the greatest Hokage! That way, people will stop disrespecting me and start treating me like I'm somebody. Someone important!" --speaker_wav ./speaker.wav --language_idx en --use_cuda true --out_path "./WAVFILES/Anime.wa
v"

Create a Folder of Speakers:
Loop Through Depending On The Quote, And Choose The Correct Speaker For the Quote
